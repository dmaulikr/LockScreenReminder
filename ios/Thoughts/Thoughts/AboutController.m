...
- (void)viewDidLoad
{
    ...
    if(description)
    {
        NSString *desc = [((TestAppDelegate *)[UIApplication sharedApplication].delegate) localizedFromString:@"description"];
        if([desc length])
    ...
    if(aboutUs)
    {
        NSString *string = [((TestAppDelegate *)[UIApplication sharedApplication].delegate) localizedFromString:@"about_us"];
        if(string)
    ...
    if(contactUs)
    {
        NSString *string = [((TestAppDelegate *)[UIApplication sharedApplication].delegate) localizedFromString:@"contact_us"];
        if(string)
    ...
    if(facebook)
    {
        NSString *string = [((TestAppDelegate *)[UIApplication sharedApplication].delegate) localizedFromString:@"share_this_app"];
        if(string)
    ...
    if(!fb)
    {
        fb = [[Facebook alloc] initWithAppId:@"299181733521981" andDelegate:self];
    }
}
...
//    [fb authorize:[NSArray arrayWithObjects: @"user_photos",@"user_videos",@"publish_stream",@"offline_access", nil]];
    NSMutableDictionary* params = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                   @"299181733521981", @"app_id",
                                   @"http://www.mobileprogressive.com/", @"link",
                                   @"http://mobileprogressive.com/images/sr_xlarge.png", @"picture",
                                   @"I am using Lock Screen Reminder for iPhone.", @"text",
                                   @"Lock Screen Reminder", @"name",
                                   @"Lock Screen Reminder", @"caption",
                                   @"I am using Lock Screen Reminder for iPhone.", @"description",
                                   nil];
